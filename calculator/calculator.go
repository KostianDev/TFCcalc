// Package calculator contains core logic for computing base materials.
package calculator
...
func ResolvePercentagesForAlloy(alloyID string, userPerc map[string]float64) (map[string]float64, error) {
	...
	// Copy userPerc so we do not mutate the original.
	fullPerc := make(map[string]float64)
	for k, v := range userPerc {
		fullPerc[k] = v
	}

	// Fill missing ingredients with defaults.
	...
	// If invalid, log a warning and return defaults.
...
func GetDefaultPercentages(alloyID string) (map[string]float64, error) {
	...
	// Adjust the first ingredient if rounding drifts from 100.
...
// sumMaterials merges two maps of {baseID: amountMB}.
...
// getBaseMaterialBreakdown expands a target into base materials.
func getBaseMaterialBreakdown(targetID string, amountMB float64, allUserPerc map[string]map[string]float64, level int) (map[string]float64, error) {
	...
	if targetData.Type == "final_steel" {
		...
		// Break down the extra ingredient.
	...
	// If intermediate alloy, raw_steel, or processed (other than "steel").
